...
static void
g_paste_sqlite_backend_read_special_values (sqlite3      *db,
                                            const guchar *key,
                                            gint64        item_id,
                                            GPasteItem   *item)
{
    ...
    /* add_special_value prepends, so walking positions backwards rebuilds the
     * item's special values in their original order. */
    if (sqlite3_prepare_v2 (db, "SELECT mime, data FROM special_values WHERE item_id = ? ORDER BY position DESC;", -1, &stmt, NULL) != SQLITE_OK)
    {
                                          ...
                                          gsize                      *size)
{
    ...
    *size = 0;
    ...
    while (sqlite3_step (stmt) == SQLITE_ROW)
    {
        ...
        g_paste_sqlite_backend_read_special_values (db, key, sqlite3_column_int64 (stmt, 0), item);
                                     ...
                                     const GPasteItem           *item)
{
    ...
    if (success && found)
    {
        ...
        success = (sqlite3_prepare_v2 (db, "DELETE FROM special_values WHERE item_id = ?;", -1, &del, NULL) == SQLITE_OK);
        ...
        success = success && g_paste_sqlite_backend_write_special_values (db, key, item_id, item);
    }
